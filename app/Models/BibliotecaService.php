...
use App\Models\DTO\EmprestimoDTO;
...
class BibliotecaService
{
    ...
    /**
     * Realiza um empréstimo manual de livro para um aluno.
     * Retorna array ['success' => bool, 'newLoan' => array|null]
     */
    public function emprestimoManual(int $livroId, int $alunoId, string $dataDevolucao): array
    {
        $this->db->beginTransaction();
        try
        {
            ...
            {
                $this->db->execute("UPDATE livros SET qtde_disponivel = qtde_disponivel - 1 WHERE id_livro = :id", [':id' => $livroId]);
                $this->db->execute(
                ...
                $newLoanId = $this->db->lastInsertId();
                $this->db->commit();
                // Buscar dados do novo empréstimo para retornar ao controller
                $newLoanData = $this->db->selectOne(
                ...
                return ['success' => true, 'newLoan' => $newLoanData];
            }
            else
            {
                $this->db->rollBack();
                return ['success' => false, 'newLoan' => null];
            }
        }
        catch (Exception $e)
        {
