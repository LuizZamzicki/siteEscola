...
class HorarioService
{
    ...
    /**
     * Retorna o nome do período da turma.
     */
    public function buscarPeriodoNomePorTurmaId(int $turmaId): ?string
    {
        $result = $this->db->query("SELECT periodo FROM turmas WHERE id_turma = :id_turma", [':id_turma' => $turmaId]);
        return !empty($result) ? $result[0]['periodo'] : null;
    }

    /**
     * Retorna o id do período pelo nome.
     */
    public function buscarPeriodoIdPorNome(string $periodoNome): ?int
    {
        $result = $this->db->query("SELECT id FROM periodos WHERE nome = :nome", [':nome' => $periodoNome]);
