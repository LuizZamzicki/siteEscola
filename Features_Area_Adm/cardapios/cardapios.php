...
if ($_SERVER['REQUEST_METHOD'] === 'POST')
{
    ...
    if ($action === 'salvar_cardapio')
    {
        $cardapio = new CardapioDTO(
            empty($_POST['cardapio-id']) ? null : (int)$_POST['cardapio-id'],
            $_POST['data'] ?? '',
            $_POST['lanche_manha'] ?? null,
            $_POST['almoco'] ?? '',
            $_POST['lanche_tarde'] ?? null
        );
        $result = $cardapioService->salvar($cardapio);
    }

    if ($action === 'excluir_cardapio')
    {
...
// Obter mensagens de feedback da URL
if (isset($_GET['msg']))
{
