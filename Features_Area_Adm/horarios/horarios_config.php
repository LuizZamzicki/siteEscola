...
if ($_SERVER['REQUEST_METHOD'] === 'POST')
{
    ...
    if ($action === 'salvar_horario_config')
    {
    ...
    elseif ($action === 'excluir_horario_config')
    {
    ...
    if (!empty($result))
    {
        ...
        // Preserve a seleção do período no redirect
        if (!empty($_POST['periodo_id']))
        {
            $redirectParams['periodo_id'] = $_POST['periodo_id'];
        }
        elseif ($selectedPeriodoId)
        {
            $redirectParams['periodo_id'] = $selectedPeriodoId;
        }
