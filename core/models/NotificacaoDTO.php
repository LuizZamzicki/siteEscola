...
class NotificacaoDTO
{
    ...
    public function getTempoRelativo(): string
    {
        ...
        if ($diferenca->y > 0) return "há " . $diferenca->y . " ano(s)";
        if ($diferenca->m > 0) return "há " . $diferenca->m . " mes(es)";
        if ($diferenca->d > 0) return "há " . $diferenca->d . " dia(s)";
        if ($diferenca->h > 0) return "há " . $diferenca->h . " hora(s)";
        if ($diferenca->i > 0) return "há " . $diferenca->i . " minuto(s)";
        return "agora mesmo";
    }
}
